...
| 20b | **Keyless deterministic proof path** | ✅ | `make judge-check` ([`scripts/judge_check.sh`](../scripts/judge_check.sh)) — backend **50** unit tests, healthy fixture **32/32** invariants, frontend lint + production build, tracked-secret scan; restores the incident fixture and leaves the tree clean. Requires no Anthropic key, no live DataHub, no GitHub token, no Phoenix, and touches no remote. Same evidence CI runs |
| 20c | **Safety envelope enforced in code** | ✅ | Evidence gate (`tools.py::t_confirm_root_cause`); human authorization before mutation (`api.py::start_repair`, `allow_repair`); repair path allowlist (`repair.py::_resolve_transform`); real execution + rebuild (`verify_repair`); unverified-patch revert (`investigator.py::_cleanup_unverified_patch`); full-suite + KPI acceptance band; bad-repair rejection (`bad_repair_rejected` eval); post-verification non-gating publication and `blackbox/fix-*`-only pushes (`publish_repair_pr`, `FIX_BRANCH_PREFIX`); DuckDB filesystem access disabled (`warehouse.py`) |
