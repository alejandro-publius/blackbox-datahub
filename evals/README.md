...
| `datahub_ablation` | LLM only | DataHub is load-bearing: with `BLACKBOX_DISABLE_DATAHUB=true` the agent's DataHub tools error out, and the run must **not** reach a correct confirmed root cause end-to-end (the evidence gates require DataHub lineage/metadata citations by design). |
